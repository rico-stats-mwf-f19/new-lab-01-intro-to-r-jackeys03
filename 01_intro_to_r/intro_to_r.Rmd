...
install them by running (**green sideways triangle**) the following three lines of codes.
...
```{bash eval=FALSE}
git config --global user.email "jricoiii@gmail.com"
git config --global user.name "Jose Rico"
```
...
Currently your project is called *Untitled Project*. Update the name of your project to be "lab-01-intro-to-r".
...
```{r load-present-data}
natality <- read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vQT4V0JbnhhXh5Ecch1CEBExXEL-MKl-J8TzfOLdFlWoAQ3oCQ2-xCPS5WKPNWraRmwEUiRFSzQclpp/pub?gid=0&single=true&output=csv")
```

The data are stored in a data frame called `natality`.
